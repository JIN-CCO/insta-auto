...
function css() {
  ...
  *{margin:0;padding:0;box-sizing:border-box;font-family:'Noto Sans CJK KR','Noto Sans KR',sans-serif;}
  ...
  .slide{width:1080px;height:1080px;position:relative;overflow:hidden;padding:80px 84px;display:flex;flex-direction:column;}
...
async function renderCarousel(topic, nextTopic, handle, outDir) {
  ...
  for (let i = 0; i < all.length; i++) {
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><style>${css()}</style></head><body>${slideHTML(all[i], ctx)}</body></html>`;
  ...
  const manifest = { id: topic.id, caption, files };
