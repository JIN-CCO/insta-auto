...
const { M_LOGO_PNG } = require('./logo_asset');

// MakeIT "M" 브랜드 로고 (단독) — 원본 로고 PNG
function makeitLogo(size = 440) {
  return `<img src="${M_LOGO_PNG}" style="height:${size}px;display:block" alt="MakeIT">`;
}

// "Magazine" 로고 잠금 — 원본 M 심볼 + "agazine" 텍스트를 한 글자처럼 이어붙임
function magazineLockup(_symColor, textColor = '#ffffff', h = 300) {
  const sym = `<img src="${M_LOGO_PNG}" style="height:${h}px;display:block" alt="M">`;
