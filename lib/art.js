...
// MakeIT "M" 브랜드 심볼 조각 — 두 기둥 + 상단 사각 점 + 하단 중앙 아치
function mSymbolInner(color) {
  return `<rect x="35" y="28" width="78" height="78" rx="2" fill="${color}"/>
    <rect x="327" y="28" width="78" height="78" rx="2" fill="${color}"/>
    <rect x="35" y="122" width="78" height="228" fill="${color}"/>
    <rect x="327" y="122" width="78" height="228" fill="${color}"/>
    <path d="M113 214 Q220 150 327 214 L327 350 L322 350 A104 104 0 0 0 118 350 L113 350 Z" fill="${color}"/>`;
}

// MakeIT "M" 브랜드 로고 (단독)
function makeitLogo(color = '#0066FF', size = 440) {
  return `<svg width="${size}" height="${Math.round(size * 372 / 400)}" viewBox="20 14 400 372" xmlns="http://www.w3.org/2000/svg">${mSymbolInner(color)}</svg>`;
}

// "Magazine" 로고 잠금 — M 심볼 + "agazine" 텍스트를 한 글자처럼 이어붙임
function magazineLockup(symColor = '#0066FF', textColor = '#ffffff', h = 300) {
  const sym = `<svg height="${h}" viewBox="35 28 370 322" xmlns="http://www.w3.org/2000/svg" style="display:block">${mSymbolInner(symColor)}</svg>`;
  return `<span class="mzlock"><span class="mzsym">${sym}</span><span class="mztxt" style="color:${textColor};font-size:${Math.round(h * 0.86)}px">agazine</span></span>`;
}
