...
// MakeIT "M" 브랜드 로고 — 두 기둥 + 상단 사각 점 + 하단 중앙 U(밸리)
function makeitLogo(color = '#0066FF', size = 555) {
  return `<svg width="${size}" height="${size}" viewBox="0 0 555 465" xmlns="http://www.w3.org/2000/svg">
    <rect x="155" y="112" width="50" height="56" rx="2" fill="${color}"/>
    <rect x="350" y="112" width="50" height="56" rx="2" fill="${color}"/>
    <path d="M155 185 L205 185 L205 244 A72 62 0 0 0 350 244 L350 185 L400 185 L400 352 L155 352 Z" fill="${color}"/>
  </svg>`;
}

// "Magazine" 로고 잠금 — M 심볼(기둥+점+U밸리) + "agazine" 텍스트를 한 글자처럼 이어붙임
function magazineLockup(symColor = '#0066FF', textColor = '#ffffff', h = 300) {
  const sym = `<svg height="${h}" viewBox="150 106 258 250" xmlns="http://www.w3.org/2000/svg" style="display:block">
    <rect x="155" y="112" width="50" height="56" rx="2" fill="${symColor}"/>
    <rect x="350" y="112" width="50" height="56" rx="2" fill="${symColor}"/>
    <path d="M155 185 L205 185 L205 244 A72 62 0 0 0 350 244 L350 185 L400 185 L400 352 L155 352 Z" fill="${symColor}"/>
  </svg>`;
  return `<span class="mzlock"><span class="mzsym">${sym}</span><span class="mztxt" style="color:${textColor};font-size:${Math.round(h * 0.9)}px">agazine</span></span>`;
}

module.exports = { getArt, makeitLogo, magazineLockup };
