...
const { getArt, makeitLogo } = require('./art');
...
function css() {
  ...
  *{margin:0;padding:0;box-sizing:border-box;font-family:'Noto Sans CJK KR','Noto Sans KR',sans-serif;word-break:keep-all;}
  ...
  /* 브랜드 아웃트로 */
  .outro{background:${ACCENT};display:flex;align-items:center;justify-content:center;}
  .outro-bg{position:absolute;top:50%;left:50%;transform:translate(-50%,-52%);opacity:0.13;pointer-events:none;}
  ...
  .outro-logo{margin-bottom:22px;line-height:0;}
  .outro-brand{color:rgba(255,255,255,0.92);font-size:27px;font-weight:800;letter-spacing:3px;margin-bottom:46px;}
  .outro-q{color:#fff;font-size:72px;font-weight:800;line-height:1.24;letter-spacing:-3px;}
  .outro-follow{display:flex;align-items:center;gap:14px;background:#fff;color:${ACCENT};font-size:40px;font-weight:900;padding:22px 54px;border-radius:999px;margin-top:54px;box-shadow:0 16px 36px rgba(0,0,0,0.24);}
  .outro-handle{color:rgba(255,255,255,0.88);font-size:27px;font-weight:700;margin-top:42px;letter-spacing:1px;}
  `;
}
...
function magPages(topic, nextTopic, handle, photos) {
  ...
  if (nextTopic) {
    ...
    pages.push({ name: `slide${String(n).padStart(2, '0')}.png`, html: `<div class="slide outro">
      <div class="outro-bg">${makeitLogo('#2E7BFF', 1000)}</div>
      <div class="outro-fg">
        <div class="outro-logo">${makeitLogo('#ffffff', 190)}</div>
        <div class="outro-brand">MAKEIT 매거진</div>
