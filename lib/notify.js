...
async function notifySlack({ title, series, no, permalink, threadsPermalink }) {
  ...
  if (permalink) fields.push({ type: 'mrkdwn', text: `*인스타*\n<${permalink}|게시물 보기>` });
  if (threadsPermalink) fields.push({ type: 'mrkdwn', text: `*스레드*\n<${threadsPermalink}|스레드에서 보기>` });

  // text: 워크플로 빌더용(변수 이름 text) + 일반 웹훅 폴백 — 전체 정보 포함
  const linkLine = permalink ? `\n*인스타:* <${permalink}|게시물 보기>` : '';
  const threadsLine = threadsPermalink ? `\n*스레드:* <${threadsPermalink}|스레드에서 보기>` : '';
  const text =
    `📸 *새 게시물 업로드*\n` +
    `*제목:* ${title}\n` +
    `*시리즈:* ${seriesText}\n` +
